...
{
    std::cout << "Headers " << request.MethodString() << ' ' << request.GetPath() << std::endl;
    if (!benchmark)
    {
    ...
    if (request.GetPath() == "/")
    {
        ...
        response->AddHeader("Server", "blaze");
...
int main(int argc, char* argv[])
{
    ...
    EventLoop loop;
    InetAddress server_addr(8000);
    HttpServer server(&loop, server_addr, "dummy");
